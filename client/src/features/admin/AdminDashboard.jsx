...
const COLORS=['#FF7A30','#F01818','#FF6A21','#C10F10','#8D0A0A','#f59e0b','#22c55e'];
const TT = { contentStyle:{background:'#1C1C1C',border:'1px solid #2a2a2a',borderRadius:8,color:'#fff'} };
        ...
        <div className="card">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={revenue.map(r=>({name:r.label?.substring(0,18),rev:r.revenue}))}>
              <XAxis dataKey="name" tick={{fill:'#A9A9A9',fontSize:10}}/><YAxis tick={{fill:'#A9A9A9',fontSize:10}}/>
        ...
        <table><thead><tr><th>Match</th><th>Date</th><th>Stadium</th><th>Status</th><th>Tickets</th><th>Actions</th></tr></thead>
          <tbody>{matches.map(m=>(
            <tr key={m._id}>
              ...
              <td style={{color:'#A9A9A9',fontSize:13}}>{fmt(m.matchDate)}</td>
              <td style={{color:'#A9A9A9'}}>{m.stadium?.name}</td>
    ...
    <div>
      ...
      <div className="card" style={{overflow:'hidden'}}><table><thead><tr><th>Match</th><th>Date</th><th>Status</th><th>Score</th><th>Action</th></tr></thead>
        <tbody>{matches.map(m=>(
          ...
          <td style={{color:'#A9A9A9'}}>{new Date(m.matchDate).toLocaleDateString()}</td>
          <td><span className={`badge badge-${m.status==='Scheduled'?'orange':m.status==='Completed'?'gray':'red'}`}>{m.status}</span></td>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20,marginBottom:20}}>
        <div className="card"><div className="section-title">Popular Matches</div>
          <ResponsiveContainer width="100%" height={220}><BarChart data={popular.map(p=>({name:`${p.homeTeam} vs ${p.awayTeam}`.substring(0,16),tickets:p.ticketCount}))}>
            <XAxis dataKey="name" tick={{fill:'#A9A9A9',fontSize:10}}/><YAxis tick={{fill:'#A9A9A9',fontSize:10}}/><Tooltip {...TT}/><Bar dataKey="tickets" fill="#FF7A30" radius={[4,4,0,0]}/>
        ...
        <div className="card"><div className="section-title">Most Active Fans</div>
          <ResponsiveContainer width="100%" height={220}><BarChart data={fans.map(f=>({name:f.fanName,tickets:f.ticketCount}))}>
            <XAxis dataKey="name" tick={{fill:'#A9A9A9',fontSize:10}}/><YAxis tick={{fill:'#A9A9A9',fontSize:10}}/><Tooltip {...TT}/><Bar dataKey="tickets" fill="#F01818" radius={[4,4,0,0]}/>
          </BarChart></ResponsiveContainer>
        </div>
        <div className="card"><div className="section-title">Top Scorers</div>
          <table><thead><tr><th>#</th><th>Player</th><th>Team</th><th>Goals</th><th>Assists</th></tr></thead>
            <tbody>{scorers.map((s,i)=><tr key={i}><td style={{fontWeight:800,color:i===0?'#FF7A30':i===1?'#A9A9A9':'#cd7f32'}}>{i+1}</td><td style={{fontWeight:600}}>{s.playerName}</td><td style={{color:'#A9A9A9'}}>{s.teamName||'—'}</td><td><span className="badge badge-orange">{s.totalGoals} ⚽</span></td><td style={{color:'#A9A9A9'}}>{s.totalAssists}</td></tr>)}</tbody>
          </table>
        </div>
        <div className="card"><div className="section-title">Training Attendance %</div>
          <ResponsiveContainer width="100%" height={220}><BarChart data={attendance.map(a=>({name:a.title?.substring(0,14),pct:Math.round(a.attendancePercent)}))}>
            <XAxis dataKey="name" tick={{fill:'#A9A9A9',fontSize:10}}/><YAxis domain={[0,100]} tick={{fill:'#A9A9A9',fontSize:10}}/><Tooltip {...TT} formatter={v=>`${v}%`}/><Bar dataKey="pct" fill="#22c55e" radius={[4,4,0,0]}/>
          </BarChart></ResponsiveContainer>
        </div>
      </div>
    </div>
