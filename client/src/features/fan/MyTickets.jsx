...
const MyTickets = () => {
  ...
  const cancel = async id => {
    ...
    setCancelling(id);
    try {
      await api.delete(`/api/fan/tickets/${id}`);
      load();
    } catch (err) {
  ...
  return (
    <div>
      ...
      {tickets.length === 0 ? (
        ...
        >
          No tickets yet. Book your first match! 🎟️
        </div>
      ) : (
          ...
          {tickets.map(t => (
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                >
                  📅 {fmt(t.match?.matchDate)} · Seat:{' '}
                  <strong style={{ color: '#FFFFFF' }}>{t.seatNumber}</strong>
                </div>
              </div>
              <div
                style={{
              ...
              >
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <span
                    className={`badge badge-${
                      t.payment?.status === 'Paid' ? 'orange' : 'gray'
                    }`}
                  >
                    {t.payment?.status}
                  </span>
                ...
                <div
                  style={{
                    fontWeight: 700,
                    color: '#A31400', // price highlight (main accent)
                  }}
                >
                  ${t.payment?.amount}
                </div>
                {t.status === 'Active' && t.match?.status === 'Scheduled' && (
                  ...
                  >
                    {cancelling === t._id ? 'Cancelling...' : 'Cancel'}
                  </button>
                )}
              </div>
            </div>
